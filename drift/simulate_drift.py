...
def generate_base_data(n: int = 30000, seed: int = 42) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        ...
        "P_emaildomain": rng.choice(
            ["gmail.com", "yahoo.com", "hotmail.com", "outlook.com", float("nan")], size=n
        ),
...
def compute_ks_drift(reference: pd.DataFrame, current: pd.DataFrame,
...
def run_drift_simulation(output_dir: str = "results/metrics",
    ...
    print(f"[drift_simulation] Early period: {len(early_df):,} rows  | "
          f"Late period: {len(late_df):,} rows")
    print(f"[drift_simulation] Early fraud rate: {early_df['isFraud'].mean():.4f} | "
    ...
    print(f"[drift_simulation] Late fraud rate after injection: "
          f"{late_df_drifted['isFraud'].mean():.4f}")
    ...
    print(f"[drift_simulation] Natural drift – {drift_natural['n_drifted']}/{drift_natural['n_features_checked']} "
    ...
    print(f"[drift_simulation] Injected drift – {drift_injected['n_drifted']}/{drift_injected['n_features_checked']} "
    ...
    for w in range(5):
        ...
        print(f"  Window {w+1}: recall={recall:.4f}  AUC={auc:.4f}  "
              f"(n={len(window_df)}, fraud_rate={y_w.mean():.4f})")
    ...
    if drift_injected["feature_scores"]:
        ...
        colors = ["tomato" if drift_injected["feature_scores"][f]["drifted"] else "steelblue"
                  for f in feats]
    ...
    report = {
        ...
        "recommendation":          (
            "Retrain model immediately." if drift_injected["max_ks_stat"] > 0.25
            else "Monitor closely – drift detected." if drift_injected["drift_detected"]
    ...
    print(f"  Natural drift   : {drift_natural['n_drifted']} features  "
          f"(max KS={drift_natural['max_ks_stat']:.4f})")
    print(f"  Injected drift  : {drift_injected['n_drifted']} features  "
          f"(max KS={drift_injected['max_ks_stat']:.4f})")
