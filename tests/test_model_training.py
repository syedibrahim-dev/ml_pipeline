...
class TestXGBoost:
    def test_xgboost_trains_without_error(self, fraud_dataset):
        ...
        d = fraud_dataset
        model = xgb.XGBClassifier(n_estimators=20, max_depth=3, verbosity=0, use_label_encoder=False, random_state=42)
    ...
    def test_xgboost_recall_above_zero(self, fraud_dataset):
        ...
        model = xgb.XGBClassifier(
            n_estimators=50, max_depth=4, scale_pos_weight=spw, verbosity=0, use_label_encoder=False, random_state=42
        )
        model.fit(d["X_train"], d["y_train"])
        preds = model.predict(d["X_test"])
    ...
    def test_cost_sensitive_improves_recall(self, fraud_dataset):
        ...
        m_std = xgb.XGBClassifier(
            n_estimators=50, scale_pos_weight=base_spw, verbosity=0, use_label_encoder=False, random_state=42
        )
        ...
        m_cs = xgb.XGBClassifier(
            n_estimators=50, scale_pos_weight=base_spw * 10, verbosity=0, use_label_encoder=False, random_state=42
        )
        m_cs.fit(d["X_train"], d["y_train"])
        recall_cs = recall_score(d["y_test"], m_cs.predict(d["X_test"]), zero_division=0)

        assert (
            recall_cs >= recall_std - 0.05
...
class TestLightGBM:
    def test_lightgbm_trains_without_error(self, fraud_dataset):
        ...
        d = fraud_dataset
        model = lgb.LGBMClassifier(n_estimators=20, max_depth=3, class_weight="balanced", random_state=42, verbose=-1)
        model.fit(d["X_train"], d["y_train"])
        assert model is not None
    ...
    def test_lightgbm_auc_above_random(self, fraud_dataset):
        ...
        model = lgb.LGBMClassifier(n_estimators=50, class_weight="balanced", random_state=42, verbose=-1)
        model.fit(d["X_train"], d["y_train"])
        proba = model.predict_proba(d["X_test"])[:, 1]
...
class TestRFHybrid:
    def test_rf_hybrid_selects_top_features(self, fraud_dataset):
        ...
        d = fraud_dataset
        rf = RandomForestClassifier(n_estimators=30, class_weight="balanced", random_state=42, n_jobs=1)
    ...
    def test_rf_hybrid_xgboost_trains_on_reduced_features(self, fraud_dataset):
        ...
        xgb_model = xgb.XGBClassifier(n_estimators=20, verbosity=0, use_label_encoder=False, random_state=42)
...
class TestSHAP:
    def test_shap_tree_explainer_runs(self, fraud_dataset):
        ...
        d = fraud_dataset
        model = xgb.XGBClassifier(n_estimators=20, verbosity=0, use_label_encoder=False, random_state=42)
        model.fit(d["X_train"], d["y_train"])
...
class TestBusinessCost:
    ...
    def test_cost_sensitive_reduces_fn_count(self, fraud_dataset):
        ...
        assert fn_cs <= fn_std + 2, f"Cost-sensitive FN ({fn_cs}) should be <= standard FN ({fn_std}) + 2 tolerance"
